/* -----------------------------------------------------
                        oled1306rot_i2c.c
                       OLED 1306 rotate I2C

    Softwaremodul fuer das Anbinden eines OLED Displays
    mit SSD1306 Controller und I2C Interface. Textaus-
    gabe erfolgt um 180 Grad gedreht (auf dem Kopf
    stehend).
...
void oled_putchar(uint8_t ch)
{
  ...
  if (doublechar)
  {
    ...
    for (i= 0; i< 8; i++)
    {
      ...
      z1= pgm_read_byte(&(font8x8h[ch-' '][7-i]));
      z1= reversebyte(z1);
      z2[i]= 0;
      for (b= 0; b< 8; b++)
      {
  ...
  else                          // nicht doublechar
  {
    ...
    for (i= 0; i< 8; i++)
    {
      outbyte= pgm_read_byte( &(font8x8h[ch-' '][7-i]) );
      outbyte= reversebyte(outbyte);
      if ((!textcolor)) i2c_write(~(outbyte) );
                   else i2c_write(outbyte);
    }
    i2c_stop();
